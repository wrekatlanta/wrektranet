...
describe TransmitterLogEntry do
  it "has a valid factory" do
    FactoryBot.create(:transmitter_log_entry).should be_valid
  end

  it "cannot have a null sign_in time" do
    FactoryBot.build(:transmitter_log_entry, sign_in: nil).should_not be_valid
  end

  it "sets sign_in to the current day" do
    tlog = FactoryBot.build(:transmitter_log_entry, sign_in: "22:00")

    expect(tlog.sign_in.to_date).to eq(DateTime.now.to_date)
  end

  describe "sign_out time is automatically set to the correct date" do
    subject { FactoryBot.create(:transmitter_log_entry, sign_in: "22:00") }
